...
void CViewRender::DrawScope( const CViewSetup &viewSet )
{
	...
	if ( localPlayer->GetActiveWeapon()->GetDynamicScopeFOV() == 0.0f )
		return;
	...
	scopeView.x = 0;
	scopeView.y = 0;
	scopeView.fov = localPlayer->GetActiveWeapon()->GetDynamicScopeFOV();
	scopeView.m_bOrtho = false;

	scopeView.m_flAspectRatio = 1.0f;

	if (/*!localPlayer->GetActiveWeapon()->IsDynamicScopeZoomed() &&*/ localPlayer->GetViewModel())
	{
		int nAttach = localPlayer->GetViewModel()->LookupAttachment( "scope_cam" );
		if (nAttach != -1)
		{
			// Set the view to the attachment
			localPlayer->GetViewModel()->GetAttachment( nAttach, scopeView.origin, scopeView.angles );
		}
	}

	//Set the view up and output the scene to our RenderTarget (Scope Material).
	Frustum frustum;
	render->Push3DView( scopeView, VIEW_CLEAR_DEPTH | VIEW_CLEAR_COLOR, pRenderTarget, (VPlane *)frustum );
	...
	ViewDrawScene( false, SKYBOX_3DSKYBOX_VISIBLE, scopeView, VIEW_CLEAR_DEPTH, VIEW_INTRO_CAMERA ); // Using VIEW_INTRO_CAMERA to indicate this is a perspective view

	//GetClientModeNormal()->DoPostScreenSpaceEffects( &scopeView );
