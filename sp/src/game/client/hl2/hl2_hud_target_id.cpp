...
CHudElement(pElementName), BaseClass(NULL, "TargetID")
{
	...
	SetParent(pParent);
	vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx(enginevgui->GetPanel(PANEL_CLIENTDLL), "resource/CoopScheme.res", "CoopScheme");
	SetScheme(scheme);
...
void CTargetID::ApplySchemeSettings(vgui::IScheme *scheme)
{
	...
	m_cColor = scheme->GetColor("PlayerColor", Color(255, 0, 0, 255));
	m_cColorShadow = scheme->GetColor("PlayerColorShadow", Color(255, 255, 255, 255));
...
#define MAX_ID_STRING 256
	...
	if (iEntIndex)
	{
		...
		if (sIDString[0])
		{
			...
			surface()->GetTextSize(m_hFont, sIDString, wide, tall);
			...
			surface()->DrawSetTextColor(m_cColor);
			surface()->DrawPrintText(sIDString, wcslen(sIDString));

			surface()->DrawSetTextPos(xpos+1, ypos+1);
			surface()->DrawSetTextColor(m_cColorShadow);
			surface()->DrawPrintText(sIDString, wcslen(sIDString));
		}
	}
}